...
def main() -> int:
    ...
    for path in instance_paths:
        ...
        if not isinstance(metrics, dict):
            ...
            continue
        # Accept primary metric with standard aggregation suffixes.
        # evaluate_instance() may aggregate over seeds and emit
        ...
        for suffix in _suffixes:
            try:
                val = float(metrics[pm + suffix])
                break
            except (KeyError, TypeError, ValueError):
                continue
        if val is None:
            tried = ", ".join(repr(pm + s) for s in _suffixes)
            failures[key] = f"primary metric not found (tried {tried})"
            continue
