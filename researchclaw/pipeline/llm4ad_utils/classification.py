...
def _build_prompt(names: list[str], exp_plan: Any) -> str:
    ...
    return (
        ...
        "Use the plan to decide; when a directory name is an identifier rather "
        "than the plan's wording, match it by the algorithm it denotes. If a "
        "directory is a grid/parameter sweep over a method, label it by the role "
        "of that underlying method. If the plan is ambiguous, judge from what the "
        "algorithm is.\n\n"
