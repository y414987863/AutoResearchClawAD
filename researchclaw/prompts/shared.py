...
_DEFAULT_BLOCKS: dict[str, str] = {
    ...
    "compute_budget": (
        ...
        "- Estimate: a single experimental run (1 seed × 1 config × 1 algorithm) typically takes 10-60 seconds\n"
        "- Keep the scale small: use 1-3 seeds, minimal instances, and only essential algorithms\n"
        "- CRITICAL: Total runs = (instances × seeds × algorithms) must fit the budget:\n"
        "  * time_budget < 300s: aim for ≤20 total runs (e.g., 4 instances × 2 seeds × 3 algorithms = 24)\n"
        "  * time_budget < 600s: aim for ≤50 total runs\n"
        "  * time_budget < 1800s: aim for ≤150 total runs\n"
        ...
        "  The deadline applies WITHIN one (condition, instance, seed) run, not to\n"
        "  the loop over conditions. A loop that breaks out of the condition list\n"
        "  loses whole conditions: the experiment then silently has no baseline and\n"
        "  every comparison it supports is invalid.\n"
        "  REQUIRED SHAPE — every condition must be attempted:\n"
        "  ```\n"
        "  total_budget = {time_budget_sec}\n"
        "  n_runs = max(1, len(ALGORITHMS) * len(instances) * len(seeds))\n"
        "  per_run_budget = total_budget / n_runs   # one slice per run\n"
        "\n"
        "  for cond in ALGORITHMS:                  # NEVER break out of this loop\n"
        "      for inst in instances:\n"
        "          for seed in seeds:\n"
        "              deadline = time.time() + per_run_budget\n"
        "              # Check the deadline INSIDE this single run, then keep\n"
        "              # whatever it produced. Giving up on one run is fine;\n"
        "              # giving up on a condition is not.\n"
        "              value = run_one(cond, inst, seed, deadline)\n"
        ...
        "  stays true. Use it at most to cut short the run you are already in —\n"
        "  never as the condition-loop's exit, which would abandon every remaining\n"
        "  condition and leave the comparison incomplete.\n"
    ...
    "hp_reporting": (
        ...
        "}\n"
        "```\n"
        "At the end of main.py, save results to results.json with this EXACT structure:\n"
        "```python\n"
        "import json\n"
        "\n"
        "# Build the nested metrics structure: metrics[algo_name][instance_id][metric_name] = value\n"
        "# Example for 2 algorithms, 2 instances:\n"
        "# metrics = {\n"
        "#   \"baseline\": {\n"
        "#     \"ackley_10d\": {\"primary_metric\": 19.26, \"primary_metric_std\": 0.13, \"runtime_sec\": 0.019},\n"
        "#     \"rastrigin_10d\": {\"primary_metric\": 81.15, \"primary_metric_std\": 2.41, \"runtime_sec\": 0.022}\n"
        "#   },\n"
        "#   \"proposed\": {\n"
        "#     \"ackley_10d\": {\"primary_metric\": 18.50, \"primary_metric_std\": 0.11, \"runtime_sec\": 0.021},\n"
        "#     \"rastrigin_10d\": {\"primary_metric\": 75.32, \"primary_metric_std\": 1.98, \"runtime_sec\": 0.024}\n"
        "#   }\n"
        "# }\n"
        "\n"
        "# Calculate the headline metric (aggregate across all algorithms and instances)\n"
        "# For example, mean of all primary_metric values:\n"
        "# headline = mean([metrics[a][i][\"primary_metric\"] for a in algos for i in instances])\n"
        "\n"
        "results = {\n"
        "    'hyperparameters': HYPERPARAMETERS,\n"
        "    'metrics': metrics,  # MUST be nested dict: metrics[algo][instance][metric_name]\n"
        "    'headline_primary_metric': headline,  # Single aggregate number for comparison\n"
        "    'per_seed': per_seed_results  # OPTIONAL: detailed per-seed data if available\n"
        "}\n"
        "\n"
        ...
        "- `metrics` MUST be a nested dict with structure: `metrics[algo_name][instance_id][metric_name] = value`\n"
        "- `headline_primary_metric` MUST be a single numeric value (the overall aggregate metric)\n"
        "- Each `metrics[algo][instance]` dict MUST contain at least the PRIMARY_METRIC\n"
        "- Do NOT use a flat list or array of results — the nested dict structure is MANDATORY\n"
        "- `per_seed` is optional but recommended for reproducibility\n"
        "\n"
    ...
    "llm4ad_task_package_guidance": (
        "\n## LLM4AD Task Package Structure (MANDATORY when llm4ad_boost is enabled)\n"
        "The generated experiment must ALSO be organized as an LLM4AD task package so "
        "downstream can build ONE evolvable package per algorithm.\n"
        "\n"
        "⚠️ CRITICAL GENERATION CHECKLIST - You MUST generate ALL of these:\n"
        "  1. ✓ main.py\n"
        "  2. ✓ evaluator.py with PRIMARY_METRIC, METRIC_DEF, evaluate_instance\n"
        "  3. ✓ At least 3 algorithms in algorithms/<algo>/<algo>.py with EVOLVE markers\n"
        "  4. ✓ At least 3 data files in data/*.json ← THIS IS MANDATORY, DO NOT SKIP\n"
        "  5. ✗ algorithms_classification.json — DO NOT generate this file\n"
        "\n"
        "❌ FAILURE TO GENERATE ANY OF THE ABOVE = INCOMPLETE GENERATION\n"
        "Your generation will be REJECTED if data/*.json files are missing.\n"
        "\n"
        "### Do NOT write algorithms_classification.json\n"
        "The pipeline classifies each algorithm directory itself, after your files "
        "exist and every name is final, and writes that file in the one shape "
        "downstream can read: "
        '{\"classification\": {\"<algo_dir_name>\": \"proposed\"|\"baseline\"|\"ablation\"}}. '
        "A file you author at that path is overwritten, and one you author earlier "
        "records names that later repair rounds replace — so it can only be wrong. "
        "Skip it entirely.\n"
        "\n"
        "What DOES help is a tree whose roles are legible: give each algorithm a "
        "distinctive directory name that says what the method is (not \"algorithm1\"), "
        "and make it obvious from main.py and the tree which directory is the method "
        "under test and which are baselines.\n"
        "\n"
        "These rules are about STRUCTURE ONLY. They fix WHERE code lives and WHICH two "
        "symbols the runner imports — they say nothing about what your problem looks like. "
        "Choose the instance fields, the algorithm's return value, the seeds, the budget "
        "and the metrics entirely from YOUR research topic; do not invent fields to satisfy "
        "this section.\n\n"
        "### Directory layout (inside the experiment directory)\n"
        "```\n"
        "main.py                      # shared entry point (see below)\n"
        "algorithms/<algo>/<algo>.py  # one file per algorithm, inside its own subdir\n"
        "evaluator.py                 # fixed evaluation logic (see contract below)\n"
        "data/*.json                  # static instances, generated once, then read-only\n"
        "```\n"
        "Any additional fixed modules your topic needs (problem definitions, statistics "
        "helpers, ...) are welcome at the experiment root — name them whatever fits. Only "
        "`evaluator.py` has a fixed name, because the runner imports it.\n\n"
        "### The ONLY required exports: `evaluator.py`\n"
        "The task package adds a thin runner that imports exactly these three names. If any "
        "is missing or renamed, every algorithm fails at package build time:\n"
        "\n"
        "- ``PRIMARY_METRIC`` — a string constant: the name of the metric to select on.\n"
        "  ```python\n"
        "  PRIMARY_METRIC = \"solution_quality\"  # your metric name\n"
        "  ```\n"
        "\n"
        "- ``METRIC_DEF`` — a static dict declaring the metric direction (MANDATORY):\n"
        "  ```python\n"
        "  METRIC_DEF = {\"primary_metric\": PRIMARY_METRIC, \"direction\": \"maximize\"}\n"
        "  ```\n"
        "  Set ``direction`` to ``\"maximize\"`` when a LARGER ``PRIMARY_METRIC`` value is "
        "better, ``\"minimize\"`` when a SMALLER value is better. Read the comment in your own "
        "metric computation (e.g. \"larger = better correction\") — it must match.\n"
        "  **CRITICAL**: Without this, direction inference may be WRONG and the promote step "
        "will select REGRESSIONS as improvements. Stage validation will FAIL if this is missing.\n"
        "  You MAY also keep a runtime ``print(\"METRIC_DEF: ...\")`` line, but the static dict "
        "is what other stages parse.\n"
        "\n"
        "- ``evaluate_instance(instance: dict, solve) -> dict[str, float]`` — run the callable "
        "``solve`` on ``instance`` and return the metrics dict for that instance. "
        "``solve`` has the algorithm signature ``solve(instance, seed)``.\n"
        "  - The returned dict MUST contain a key named ``PRIMARY_METRIC`` (aggregate "
        "variants such as ``<PRIMARY_METRIC>_mean``/``_std`` may be included alongside it).\n"
        "  - **This function owns EVERYTHING domain-specific**: which seeds to repeat over, "
        "how to derive per-run randomness, any evaluation budget, any per-algorithm "
        "hyperparameters, how to read the algorithm's return value, and how to aggregate. "
        "The runner does none of that — it only loads the algorithm and calls you.\n"
        "  - It must be deterministic and non-time-sensitive.\n"
        "- ``load_instance(path) -> instance`` — OPTIONAL, and required ONLY if your "
        "instance files are not JSON. The runner reads each file in ``data/`` and, "
        "without this hook, can only ``json.load`` it. If your problem's standard "
        "format is TSPLIB, ``.mps``, ``.npz``, ``.csv`` or anything else, define this "
        "function to parse one file into whatever object ``evaluate_instance`` expects. "
        "Parsing is a domain decision, so it belongs here, not in the runner.\n"
        "- **The `instance` dict you pass to `solve` is the SAME object you received.** "
        "If an algorithm reads `instance[\"k\"]` (e.g. it builds a cost matrix itself AND "
        "the algorithm indexes `instance[\"cost\"]`), then EITHER the key is already in "
        "`data/*.json`, OR `evaluate_instance` must write it back before calling `solve` "
        "(``instance[\"cost\"] = generate_cost_matrix(instance)``). Computing a value into "
        "a local variable is NOT enough — the algorithm only sees the dict you hand it, "
        "so a locally-built `cost` that is never assigned to `instance` makes every "
        "algorithm fail with `KeyError`. This is the single most common reason an "
        "experiment scores `inf` and looks like a modelling failure.\n"
        "- Consequence to check before you finish: `evaluate_instance` must work given ONLY "
        "an instance loaded from `data/*.json` and the algorithm callable. If it needs a "
        "budget or a hyperparameter, that value must come from the instance file or from "
        "`evaluator.py` itself — NEVER from `main.py`, which the runner never imports.\n\n"
        "### Algorithm module (`algorithms/<algo>/<algo>.py`)\n"
        "- The file name MUST match its directory name: `algorithms/<algo>/<algo>.py`.\n"
        "- Exactly ONE evolvable function, with the shared signature:\n"
        "```python\n"
        "def optimize(instance: dict, seed: int) -> dict:\n"
        "    ...\n"
        "```\n"
        "  The NAME `optimize` is a fixed convention so the runner can find it; what it "
        "returns is entirely up to your topic, since only your `evaluate_instance` reads it.\n"
        "- Wrap the ENTIRE `optimize` function — the `def` line through its final `return` — "
        "between `EVOLVE_START` and `EVOLVE_END` markers, so the whole function is the "
        "evolvable unit:\n"
        "```python\n"
        "# EVOLVE_START\n"
        "def optimize(instance: dict, seed: int) -> dict:\n"
        "    ...            # the whole algorithm, start to finish\n"
        "    return {...}\n"
        "# EVOLVE_END\n"
        "```\n"
        "- **NEVER rename `optimize` or change its signature** (`instance, seed`): the "
        "runner and evaluator import it by that exact name and call it with those exact "
        "arguments. The LLM4AD mutation step may rewrite the function body but must keep "
        "the function name and parameter list identical.\n"
        "- **The complete algorithm MUST live inside that one function.** Do NOT define a "
        "helper class or function in the same file and call it from the marked region — a "
        "block that only constructs `MyOptimizer(...)` and calls `.run()` leaves the real "
        "algorithm outside the evolvable surface, so evolution can do nothing but retune "
        "constructor arguments. Inline the loop, the update rule, the bookkeeping, "
        "everything.\n"
        "- Genuinely SHARED logic (objective functions, problem definitions, distance or "
        "statistics helpers used by several algorithms) belongs in a fixed module at the "
        "experiment root and is imported — that keeps it identical across algorithms instead "
        "of duplicating it into each evolvable block.\n"
        "- Only these may sit outside the markers in an algorithm file: imports and "
        "module-level constants. The `def optimize(...)` line and its docstring go INSIDE "
        "the markers.\n"
        "- EVOLVE markers belong ONLY in `algorithms/<algo>/<algo>.py`. Never put an "
        "`EVOLVE_START`/`EVOLVE_END` pair in `evaluator.py`, `main.py`, another fixed module, "
        "or any `data/*.json` — the downstream `evolve_detector` treats every marker as a "
        "separate evolvable candidate, so a stray marker turns a fixed module into an "
        "evolvable target too, corrupting the run.\n\n"
        "### Static instance data (`data/*.json`) ⚠️ CRITICAL - MUST GENERATE\n"
        "- You MUST generate at least 3 instance files (data/instance1.json, data/instance2.json, data/instance3.json)\n"
        "- Each file encodes ONE evaluation case. Its fields are yours to choose.\n"
        "- **Hard requirement: every key an algorithm or `evaluate_instance` indexes "
        "unconditionally (`instance[\"k\"]`) MUST be present in EVERY instance file**, unless "
        "`evaluate_instance` injects it before calling the algorithm. Downstream validation "
        "extracts those keys from your source and checks the JSON — a missing key is a "
        "`KeyError` that fails every algorithm at evaluation time.\n"
        "- Use `instance.get(\"k\", default)` for anything genuinely optional.\n"
        "- Keep the key spelling consistent across every algorithm and every instance file.\n"
        "- If the data directory already exists, treat the files as READ-ONLY and use them "
        "as-is. If not, generate them deterministically and include the JSON in your output.\n"
        "- Never regenerate or alter instance files on a re-run.\n"
        "\n"
        "MINIMAL COMPLETE EXAMPLE (for reference):\n"
        "The point of these files is that EVERY instance exposes the SAME keys, so a\n"
        "given algorithm can index them unconditionally. Field names are generic and\n"
        "hold no domain meaning — use whatever your problem actually needs.\n"
        "```filename:data/instance1.json\n"
        "{\n"
        "  \"instance_id\": \"inst_1\",\n"
        "  \"n\": 5,\n"
        ...
        "}\n"
        "```\n"
        "```filename:data/instance2.json\n"
        "{\n"
        "  \"instance_id\": \"inst_2\",\n"
        "  \"n\": 5,\n"
        "  \"params\": {\"alpha\": 1.5, \"beta\": 0.0}\n"
        "}\n"
        "```\n"
        "```filename:data/instance3.json\n"
        "{\n"
        "  \"instance_id\": \"inst_3\",\n"
        "  \"n\": 5,\n"
        ...
        "}\n"
        "```\n"
        "Replace the field names with YOUR problem's actual keys, keeping them\n"
        "consistent across ALL instance files. The above is just an example.\n"
        "The key point: generate AT LEAST 3 files with problem-specific data.\n\n"
        "### Entry point (`main.py`)\n"
        "- `main.py` MUST be the runnable experiment entry point, ending with an explicit "
        "`if __name__ == \"__main__\":` block that calls `main()` (or the equivalent top-level "
        "driver). A file with only helper functions and NO entry point is BROKEN.\n"
        "- Accept an optional `--algorithm <name>` argument. When omitted, run ALL algorithms "
        "(the full stage-12 comparison, and write `results.json`). When given, run ONLY that "
        "algorithm on ONE instance. Also accept an optional `--instance <path>` selecting "
        "which `data/*.json` to use.\n"
        "- Load the selected algorithm module DYNAMICALLY by name (`importlib`), never by "
        "static import of every algorithm file.\n"
        "- Compute the instance-level metrics by CALLING `evaluator.evaluate_instance` — do "
        "NOT re-implement that logic. The package runner calls the SAME function, so the "
        "number the paper reports equals the number LLM4AD optimises.\n"
        "- ALWAYS print to stdout: `{\"primary_metric\": <metric_name>, \"metrics\": {...}}`.\n"
        "- In the default (all) mode, ALSO write `results.json` with hyperparameters + metrics.\n\n"
        "### Evaluation separation (this is what makes the comparison trustworthy)\n"
        "- Evaluation data and evaluation logic MUST live OUTSIDE the `algorithms/` "
        "directory. Downstream, `algorithms/<algo>/` is copied into a per-individual git "
        "worktree and is the ONLY thing an evolved candidate may change — anything you put "
        "in there becomes evolvable, so an evaluator placed there would let a candidate "
        "rewrite its own scoring.\n"
        "- The primary metric MUST be instance-level, deterministic (fixed seeds), and NOT "
        "wall-clock dependent — evolution would otherwise select on timing noise.\n\n"
        "### Demo scale & validation (MANDATORY for this run)\n"
        "- This is a FUNCTIONAL DEMO, not a production study. Keep the number of repeats, "
        "the per-run workload, and the number of instance files small enough that a full run "
        "completes in a few SECONDS. Do NOT inflate scale for statistical rigor.\n"
        "- After generating, smoke-test **EVERY** algorithm with one single-instance run, "
        "i.e. `main.py --algorithm <algo>` for each `<algo>` in your list, using the "
        "interpreter your task instructions name (never a bare `python` — it is usually a "
        "different environment without the packages listed above). Do NOT "
        "smoke-test only a subset — an unsmoked algorithm may be broken.\n"
        "- The ONLY valid completion check is the DEFAULT (no `--algorithm`) run — "
        "the pipeline executes THIS path at Stage 12. It must exit 0 and print a finite "
        "`primary_metric`. If it fails, FIX and re-run it until it passes; use "
        "`--algorithm <name>` only to locate which algorithm is at fault, never as a "
        "substitute for a passing default.\n\n"
        "### Self-consistency check (run AFTER generating, BEFORE declaring done)\n"
        "- `main.py` HAS `if __name__ == \"__main__\":` calling `main()` (or a named driver).\n"
        "- `evaluator.py` exports `PRIMARY_METRIC` and `evaluate_instance` — the package "
        "runner imports them by name, and nothing else.\n"
        "- For EACH `algorithms/<algo>/<algo>.py`, collect every `instance[\"k\"]` it indexes "
        "unconditionally and confirm the key exists in EVERY `data/*.json` (or is injected by "
        "`evaluate_instance`). This is the single most common cause of a package that builds "
        "but scores nothing.\n"
        "- Load one instance JSON and one algorithm module, call "
        "`evaluate_instance(instance, algo.optimize)`, and confirm the returned dict contains "
        "a finite value under `PRIMARY_METRIC`.\n\n"
        "### ⚠️ CRITICAL VALIDATION CHECKLIST — verify these or stage validation WILL FAIL\n"
        "\n"
        "**evaluator.py (MANDATORY exports — all three required):**\n"
        "```python\n"
        ...
        "```\n"
        "\n"
        "**algorithms/<algo>/<algo>.py (every algorithm file):**\n"
        "```python\n"
        "# EVOLVE_START\n"
        "def optimize(instance: dict, seed: int) -> dict:\n"
        ...
        "# EVOLVE_END\n"
        "```\n"
        "- Keep the function name `optimize` and signature `(instance, seed)` unchanged.\n"
        "\n"
        "**data/*.json:**\n"
        "- At least one instance file exists\n"
        "- Every key that algorithms index unconditionally exists in every file\n"
        "\n"
        "**main.py:**\n"
        "```python\n"
        "if __name__ == \"__main__\":\n"
        "    # --algorithm argument (argparse or similar)\n"
        "    # importlib for dynamic algorithm loading\n"
        "    # calls evaluator.evaluate_instance(instance, algo.optimize)\n"
        "    # prints: {\"primary_metric\": \"<name>\", \"metrics\": {...}}\n"
        "```\n"
        "\n"
        "If ANY of the above is missing, Stage-10 validation will reject the code and enter "
        "a repair loop. The three evaluator.py exports are the most commonly missed items.\n"

        "### Reproducibility\n"
        "- Same seed + same instance + same algorithm must reproduce the primary metric exactly.\n"
        "- Derive per-run randomness deterministically from the instance's own identifying "
        "fields plus the seed; never rely on Python's randomized `hash()` or on "
        "build-order-dependent iteration.\n"
        "- Only the `algorithms/` directory content is the evolvable unit — everything else is fixed.\n"
    ),
    "rl_step_guidance": (
