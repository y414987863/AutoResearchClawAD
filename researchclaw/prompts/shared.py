...
_DEFAULT_BLOCKS: dict[str, str] = {
    ...
    "llm4ad_task_package_guidance": (
        ...
        "The task package adds a thin runner that imports exactly these two names. If either "
        ...
        "  - **ALSO define a module-level static direction declaration** so downstream stages "
        "agree on which way is better WITHOUT running the code (they never execute your "
        "generated evaluator). Add a constant at module level:\n"
        "    ```python\n"
        ...
        "metric computation (e.g. \"larger = better correction\") — it must match. You MAY also "
        "keep a runtime ``print(\"METRIC_DEF: ...\")`` line, but the static dict is what other "
        "stages parse.\n"
        ...
        "- Wrap the ENTIRE body of `optimize` between `EVOLVE_START` and "
        "`EVOLVE_END` markers:\n"
        "```python\n"
        "def optimize(instance: dict, seed: int) -> dict:\n"
        "    # EVOLVE_START\n"
        ...
        "- Only these may sit outside the markers in an algorithm file: imports, module-level "
        "constants, and the `def optimize(...)` line with its docstring.\n"
        ...
        "### Static instance data (`data/*.json`)\n"
