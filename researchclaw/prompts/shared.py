...
_DEFAULT_BLOCKS: dict[str, str] = {
    ...
    "llm4ad_task_package_guidance": (
        ...
        "The generated experiment must ALSO be organized as an LLM4AD task package "
        "so downstream can build ONE evolvable package per algorithm. This is the "
        "structure the LLM4AD evolve_detector relies on; deviations break the build.\n\n"
        ...
        "benchmarks.py                  # fixed evaluation data (benchmark funcs, bounds, f*)\n"
        "evaluator.py                   # fixed evaluation logic (metrics, aggregation)\n"
        "stats_utils.py                 # fixed statistics helpers\n"
        "data/<func>_d<dim>_s<seed>.json  # static instances, generated once, then read-only\n"
        "```\n\n"
        "### Required module exports (the downstream runner imports these by name)\n"
        "The LLM4AD task package adds a runner that imports these EXACT symbols. If any "
        "is missing or has a different name, every algorithm fails to evaluate with an "
        "``ImportError`` at package build time — so fix the contract HERE, not later:\n"
        "- ``benchmarks.py`` MUST define ``get_bounds(func_name: str, dim: int) -> (lb, ub)`` "
        "returning numpy arrays (lower/upper box bounds for that function/dimension).\n"
        "- ``stats_utils.py`` MUST define:\n"
        "  - ``SEEDS`` — a non-empty tuple/list of int seeds iterated to average over.\n"
        "  - ``mean_std(values) -> (mean, std)`` — numpy mean/std of a list of floats.\n"
        "  - ``rng_from_instance(func_name, dim, seed, start) -> numpy Generator`` — "
        "a deterministic RNG derived from those four arguments.\n"
        "- ``evaluator.py`` MUST define the instance-level metric as a single shared "
        "entry point that BOTH ``main.py`` and the package runner call, so the number the "
        "paper reports equals the number LLM4AD optimises:\n"
        "  - ``PRIMARY_METRIC`` — the metric-name string constant.\n"
        "  - ``prepare_start(instance, seed, start_idx) -> dict`` — returns a copy of the "
        "instance with ``lb``/``ub`` (from the instance, else ``get_bounds``), a "
        "deterministic ``x0`` (via ``rng_from_instance``), and the ``seed``/``start_idx`` "
        "injected. `optimize` reads ``instance['x0']``.\n"
        "  - ``evaluate_instance(per_seed_metric: dict) -> dict`` — given per-seed values, "
        "returns the metrics dict (mean/std). Keep it deterministic and non-time-sensitive.\n\n"
        "### Algorithm module (`algorithms/<algo>/<algo>.py`)\n"
        "- The file name MUST match its directory name: `algorithms/<algo>/<algo>.py`.\n"
        "- Exactly ONE evolvable function, following the shared signature:\n"
        "```python\n"
        "def optimize(instance: dict, seed: int) -> dict:\n"
        "    ...\n"
        "```\n"
        "- Wrap ONLY the optimize implementation between `EVOLVE_START` and `EVOLVE_END` markers:\n"
        "```python\n"
        "def optimize(instance: dict, seed: int) -> dict:\n"
        "    # EVOLVE_START\n"
        "    ...\n"
        ...
        "- Do NOT put data loading, metric computation, or stats inside the evolve block.\n"
        "- EVOLVE markers belong ONLY in `algorithms/<algo>/<algo>.py`. Never put an "
        "`EVOLVE_START`/`EVOLVE_END` pair in `benchmarks.py`, `evaluator.py`, `stats_utils.py`, "
        "`main.py`, or any `data/*.json` — the downstream `evolve_detector` treats every "
        "marker as a separate evolvable candidate, so a stray marker in a fixed module turns "
        "that fixed module into an evolvable target too, corrupting the run.\n\n"
        "### Static instance data (`data/*.json`)\n"
        "- Each instance file encodes one evaluation case as `(function, dim, seed)`.\n"
        "- **The JSON fields MUST match what the runner and each `optimize(instance, seed)` "
        "actually consume.** The dimension key is `dim` (singular, lowercase) — NEVER "
        "`dimension`, `dims`, or `n_dim`; the downstream runner reads `instance['dim']` "
        "unconditionally, so any other key fails every algorithm at evaluation time. "
        "Before writing data, READ `evaluator.py` and `algorithms/<algo>/<algo>.py` "
        "and include EVERY field they access (e.g. `function`, `dim`, `lb`, `ub`, "
        "`x0`/`starts`, `max_evals`, `seed`). A data file missing a field the evaluator reads "
        "causes `KeyError` and makes every algorithm FAIL to evaluate.\n"
        "- If the data directory already exists, treat the files as READ-ONLY and use them as-is.\n"
        "- If it does not exist, generate it deterministically from the fixed benchmark "
        "definitions and include the JSON files in the returned code.\n"
        "- Never regenerate or alter instance files on a re-run.\n\n"
        "### Entry point (`main.py`)\n"
        "- `main.py` MUST be the runnable experiment entry point. It MUST end with an explicit "
        "`if __name__ == \"__main__\":` block that calls `main()` (or the equivalent top-level "
        "driver). A file containing only helper functions/benchmark defs and NO entry point is "
        "BROKEN and unusable. Put raw benchmark definitions in `benchmarks.py`, NOT `main.py`.\n"
        ...
        "- Compute the instance-level primary metric by CALLING the fixed `evaluator.py` "
        "helpers (`prepare_start`, `evaluate_instance`) — do NOT re-implement that "
        "aggregation. The downstream package runner uses the SAME `evaluator.py`, so the "
        ...
        "- Put evaluation data (`benchmarks.py`) and evaluation logic (`evaluator.py`, "
        "`stats_utils.py`) OUTSIDE the `algorithms/` directory so they are never evolved.\n"
        "- The primary metric MUST be instance-level, non-time-sensitive, and deterministic "
        "(fixed seed; no wall-clock-dependent values as the primary metric).\n"
        "- The same `evaluator.py` is shared by the default-all run, the single-algorithm run, "
        "and the package runner, so metrics stay comparable.\n\n"
        "### Demo scale & validation (MANDATORY for this run)\n"
        "- This is a FUNCTIONAL DEMO, not a production study. Pick a SMALL default "
        "configuration: keep the number of random starts, the per-start evaluation budget, "
        "and the number of instance files small enough that a full run completes in a few "
        "SECONDS. Do NOT inflate scale for statistical rigor.\n"
        "- After generating, smoke-test **EVERY** algorithm with one single-instance run, "
        "e.g. `python main.py --algorithm <algo>` for each `<algo>` in your algorithms list. "
        "Every algorithm must return a finite `best_f` within its evaluation budget; do NOT "
        "smoke-test only a subset, because an unsmoked algorithm may be broken.\n"
        "- NEVER run the default (no `--algorithm`) full comparison during generation — that "
        "full run belongs to a LATER stage, not here.\n"
        "- If the default (no `--algorithm`) mode is run at all, it MUST finish in a few "
        "seconds, which the small default config guarantees.\n\n"
        "### Runtime correctness (MANDATORY for every algorithm)\n"
        "- Every `optimize` call MUST return `n_evals <= max_evals` (never over-budget) and a "
        "FINITE `best_f` (never `inf`/`nan`). A run that exceeds its budget or returns a "
        "non-finite objective is silently treated as a FAILED run and dropped from the metrics, "
        "which corrupts the comparison.\n"
        "- When a line search / golden-section search already evaluated a candidate point, "
        "REUSE that value. Do NOT re-evaluate the same point and count it again — double "
        "counting a single objective evaluation pushes `n_evals` over the budget.\n"
        "- Guard the returned `n_evals` (e.g. clamp to `max_evals`) and ensure `best_f` is "
        "finite before returning.\n\n"
        ...
        "- `benchmarks.py` exports `get_bounds`; `stats_utils.py` exports `SEEDS`, `mean_std`, "
        "`rng_from_instance`; `evaluator.py` exports `PRIMARY_METRIC`, `prepare_start`, "
        "`evaluate_instance` — the package runner imports them by name.\n"
        "- For a sample `data/*.json`, load one algorithm module and confirm its "
        "`optimize(instance, seed)` returns `best_f` (finite) and `n_evals` "
        "(<= max_evals). The package runner (``run_single.py``) owns the seeds/starts "
        "loop and calls `prepare_start` to re-derive `x0`, so `instance` must carry at "
        "least `function`, `dim`, `lb`, `ub`, `max_evals`.\n"
        "### Reproducibility\n"
        "- Same seed + same instance + same algorithm must reproduce the primary metric exactly.\n"
        "- Derive per-run randomness from `(function, dimension, seed, start)` with a stable "
        "hash; never rely on Python's randomized `hash()` or build-order-dependent iteration.\n"
