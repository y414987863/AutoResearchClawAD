...
STAGES["code_generation"] = {
    ...
    "user": (
        ...
        "`condition=<name> {metric}: <value>` for EACH condition. Note the "
        "COLON-AND-SPACE before the value, not `=`.\n"
        "4. After all conditions, print one summary line PER condition:\n"
        "   `SUMMARY condition=<name> metric={metric} mean=<M> std=<S>`\n"
        "   (no colon after SUMMARY, no commas, one condition per line).\n"
