...
def test_smoke_timeout_flagged():
    # A timeout is no longer silently treated as success: it must surface as a
    # (rc, tail, timed_out=True) tuple so the repair loop can nag about a hang.
    def to(args, **kw):
        raise __import__('subprocess').TimeoutExpired(args, 120)
    with patch('subprocess.run', to):
        r = cg._try_smoke_run(_make('x=1\n'), _Cfg())
    assert r is not None and r[2] is True and r[0] == 0
